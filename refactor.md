...
- **图像层跳过**：`ImgViewerRenderer` 用 `ImageLayerKey`（bitmap 身份 + 所有缩放/平移/旋转/翻转/采样/edit 旋转/棋盘/dpi/viewport 标量）判定图像层输入是否变化，未变则跳过昂贵的大位图 blit。这正是用户场景（大图 + 编辑高速拖动）的主成本——拖动绘制时图像不变，图像层不再每帧重画。动画每帧 bitmap 对象不同 → key 变 → 正常重画（已核实无原地像素更新）。
- **编辑层跳过**：非编辑态（普通看图，绝大多数会话）在清空一帧后停止重画编辑叠加层；编辑中始终重画（笔画/选区每帧变）。
- **UI overlay 跳过：受阻，未做**。UI overlay（工具条 + Info Panel 直方图）内容由 `RenderImgViewer` 每帧 push，无法廉价判定“未变”。正解需 Step 5（信号 + 相等跳过驱动主 UI），或按 §3.4 把 Info Panel 单独提到一层。属更大改动，未在本专项内。
